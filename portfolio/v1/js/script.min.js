...
$parent.removeClass('dropdown--open').trigger($.Event('hidden.nth.dropdown',relatedTarget))})}Dropdown.prototype.toggle=function(e){var $this=$(this)
if($this.is(':disabled'))return
var $parent=getParent($this)
var isActive=$parent.hasClass('dropdown--open')
clearMenus()
if(!isActive){if('ontouchstart'in document.documentElement){$(document.createElement('div')).addClass('dropdown-backdrop').insertAfter($(this)).on('click',clearMenus)}var relatedTarget={relatedTarget:this}
$this.trigger('focus').attr('aria-expanded','true')
$parent.toggleClass('dropdown--open').trigger($.Event('shown.nth.dropdown',relatedTarget))}return false}
...
return $this.trigger('click')}var $items=$parent.find('.dropdown__menu a')
...
return this}
$(document).on('click.nth.dropdown.data-api',clearMenus).on('click.nth.dropdown.data-api','.dropdown form',function(e){e.stopPropagation()}).on('click.nth.dropdown.data-api',toggle,Dropdown.prototype.toggle).on('keydown.nth.dropdown.data-api',toggle,Dropdown.prototype.keydown).on('keydown.nth.dropdown.data-api','.dropdown__menu',Dropdown.prototype.keydown)}(jQuery);document.addEventListener('DOMContentLoaded',function(){document.addEventListener('click',function(event){if(event.target.dataset.toggle==='off-canvas'){event.preventDefault();offCanvasToggle();}if(event.target.dataset.toggleNative==='off-canvas'){offCanvasToggle();}});function offCanvasToggle(){document.getElementById('off-canvas').classList.toggle('off-canvas--open');}});+function($){'use strict';var Modal=function(element,options){this.options=options
...
this.$dialog.on('mousedown.dismiss.nth.modal',function(){that.$element.one('mouseup.dismiss.nth.modal',function(e){if($(e.target).is(that.$element))that.ignoreBackdropClick=true})})
this.backdrop(function(){if(!that.$element.parent().length){that.$element.appendTo(that.$body)}that.$element.css('display','flex').scrollTop(0)
that.adjustDialog()
...
this.$element.removeClass('modal--open').off('click.dismiss.nth.modal').off('mouseup.dismiss.nth.modal')
this.$dialog.off('mousedown.dismiss.nth.modal')
this.hideModal()}
Modal.prototype.enforceFocus=function(){$(document).off('focusin.nth.modal').on('focusin.nth.modal',$.proxy(function(e){if(document!==e.target&&this.$element[0]!==e.target&&!this.$element.has(e.target).length){this.$element.trigger('focus')}},this))}
Modal.prototype.escape=function(){if(this.isShown&&this.options.keyboard){this.$element.on('keydown.dismiss.nth.modal',$.proxy(function(e){e.which==27&&this.hide()},this))}else if(!this.isShown){this.$element.off('keydown.dismiss.nth.modal')}}
Modal.prototype.resize=function(){if(this.isShown){$(window).on('resize.nth.modal',$.proxy(this.handleUpdate,this))}else{$(window).off('resize.nth.modal')}}
...
if(this.isShown&&this.options.backdrop){this.$backdrop=$(document.createElement('div')).addClass('modal__backdrop ').appendTo(this.$body)
this.$element.on('click.dismiss.nth.modal',$.proxy(function(e){if(this.ignoreBackdropClick){this.ignoreBackdropClick=false
return}if(e.target!==e.currentTarget)return
this.options.backdrop=='static'?this.$element[0].focus():this.hide()},this))
if(!callback)return
callback()}else if(!this.isShown&&this.$backdrop){this.$backdrop.removeClass('modal--open')
